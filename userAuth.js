...
const authorizedRoles = process.env.AUTHORIZED_WORDPRESS_ROLES.split(/,\s?/g);
...
if (!authStrategies.includes(authStrategy)) {
  log.warn(
    `Invalid oauth strategy ${authStrategy} specific, defaulting to google auth`
  );
  authStrategy = "google";
}
...
router.get(
  "/login",
  passport.authenticate(authStrategy, isSlackOauth ? {} : googleLoginOptions)
);
...
router.use(async (req, res, next) => {
  ...
  if (req.isAuthenticated()) {
    ...
    try {
      const wpResponse = await request({
        url: "https://www.michigandaily.com/wp-json/tmd/v1/user/role",
        params: {
          email: userEmail,
        },
        headers: {
          Authorization: process.env.WORDPRESS_API_AUTH_HEADER,
        },
      });

      if (
        isAuthorized(passportUser) &&
        wpResponse.data.some((r) => authorizedRoles.includes(r))
      ) {
        setUserInfo(req);
        return next();
      } else {
